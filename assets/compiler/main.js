...
req.open('GET', "../assets/compiler/mono-config.json", true); //getting the config file that lists all the resources the roslyn compiler needs
req.onload = function () {
    ...
    for (var i = 0; i < jsonResponse.assets.length; i++) {
        if (jsonResponse.assets[i].behavior == 'assembly' && jsonResponse.assets[i].name.includes(".dll")) {
            ...
            http.open("GET", "../assets/compiler/managed/".concat(jsonResponse.assets[i].name));
